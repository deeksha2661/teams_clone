...
export default class StreamRenderer extends React.Component {
    ...
    render() {
        return (
            <div id={this.componentId} className={`ms-Grid-col m-1 ms-sm-12 ms-lg12 ms-xl12 ${this.stream.mediaStreamType === 'ScreenSharing' ? `ms-xxl12` : `ms-xxl4`}`}>
                <div class={`video-box-${this.stream.mediaStreamType === 'ScreenSharing' ? `ss` : `normal`}`} >

                    <div className={`try ${this.state.isSpeaking ? `speaking-border-for-video` : ``}`}
                        id={this.videoContainerId}>
                    </div>
                    <div className="video-title text-truncate">
